...
app.post('/cores', async (c) => {
  ...
  try {
    const res = await c.env.DB.prepare(
      `INSERT INTO cores (nome, hex, ativo, ordem) VALUES (?, ?, ?, ?)`
    ).bind(nome, hex, ativo, ordem).run();
    const id = res.meta?.last_row_id;
    const u = c.get('user');
    await audit(c.env.DB, u?.login || 'system', 'cores', 'INSERT', String(id), { nome, hex });
    const row = await c.env.DB.prepare(
      `SELECT id, nome, hex, ativo, ordem, criado_em, atualizado_em FROM cores WHERE id = ?`
    ).bind(id).first();
  ...
  try {
    const r = await c.env.DB.prepare(
      `UPDATE cores
       ...
       WHERE id = ?`
    ).bind(nome, hex, ativo, ordem, id).run();
...
app.delete('/cores/:id', async (c) => {
  ...
  if (!id) return c.json(fail('ID inválido.'), 400);
  try {
    const r = await c.env.DB.prepare(`DELETE FROM cores WHERE id = ?`).bind(id).run();
    if (!r.meta?.changes) return c.json(fail('Cor não encontrada.'), 404);
    const u = c.get('user');
...
app.delete('/cores', async (c) => {
  ...
  try {
    const r = await c.env.DB.prepare(`DELETE FROM cores`).run();
    const u = c.get('user');
...
app.post('/cores/import', async (c) => {
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    try {
      if (mode === 'overwrite') {
        // Procura por nome OU hex iguais (case-insensitive)
        const existing = await c.env.DB.prepare(
          `SELECT id FROM cores WHERE nome = ? COLLATE NOCASE OR hex = ? COLLATE NOCASE LIMIT 1`
        ).bind(nome, hex).first<{ id: number }>();
        if (existing?.id) {
          await c.env.DB.prepare(
            `UPDATE cores SET nome = ?, hex = ?, ativo = ?, atualizado_em = CURRENT_TIMESTAMP WHERE id = ?`
          ).bind(nome, hex, ativo, existing.id).run();
          updated++;
          continue;
        }
      }
      try {
        await c.env.DB.prepare(
          `INSERT INTO cores (nome, hex, ativo) VALUES (?, ?, ?)`
        ).bind(nome, hex, ativo).run();
        inserted++;
      } catch (e: any) {
        const msg = String(e?.message || e);
